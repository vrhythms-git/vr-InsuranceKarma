     ...
     // console.log('New Premium Data from ngrx in footer is ' + JSON.stringify(data))
      if (data != undefined && data != {}) {
        try{
          ...
          // else 
              this.newPremium = (this.WithoutHome + parseInt(data.calculatedPremium.data.newPremium));

              this.chart_lifePer = parseInt((( 3200 / this.newPremium) * 100).toString())
              this.chart_homePer = parseInt((( data.calculatedPremium.data.newPremium / this.newPremium) * 100).toString())
